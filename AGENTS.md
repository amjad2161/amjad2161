...
This repository is **BRAINIAC AI v1.0.0** — a Python FastAPI monolith. All nine core modules and the Watchdog supervisor run in-process inside a single uvicorn server. The sibling repo at `../Work` is a placeholder (README only); no services there.
...
Copy `.env.example` to `.env` when you need real NeuroCore `/api/v1/think*` calls.

### Lint, typecheck, and tests

From `/agent/repos/amjad2161` (or `repos/amjad2161` from workspace root):
