...
async def diagnostics():
    ...
    return {
        ...
        "reel_maker": modules["reel"].diagnostics(),
    }


def _reel_job_response(job: ReelJob) -> ReelJobResponse:
    return ReelJobResponse(
        ...
        script=job.script.to_dict() if job.script else None,
...
async def reel_platforms():
    ...
    return {"platforms": reel.list_platform_specs()}


@app.get("/api/v1/reel/trends", tags=["REEL-MAKER"])
async def reel_trends(niche: str = "general"):
    reel: ReelMaker = _modules()["reel"]
    return reel.get_trends(niche)


@app.post("/api/v1/reel/compose", response_model=ReelJobResponse, tags=["REEL-MAKER"])
async def compose_reel(req: ReelComposeRequest):
    reel: ReelMaker = _modules()["reel"]
    try:
        platforms = [Platform(p) for p in req.platforms]
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=f"Invalid platform: {exc}") from exc
    hook = HookType(req.hook_type) if req.hook_type else None
    job = await reel.compose(
        req.topic,
        style=ReelStyle(req.style),
        platforms=platforms,
...
@app.get("/api/v1/reel/jobs", response_model=list[ReelJobResponse], tags=["REEL-MAKER"])
async def list_reel_jobs(limit: int = 20):
    reel: ReelMaker = _modules()["reel"]
    return [_reel_job_response(j) for j in reel.list_jobs(limit=limit)]


@app.get("/api/v1/reel/jobs/{job_id}", response_model=ReelJobResponse, tags=["REEL-MAKER"])
async def get_reel_job(job_id: str):
    reel: ReelMaker = _modules()["reel"]
...
@app.post("/api/v1/reel/jobs/{job_id}/publish", tags=["REEL-MAKER"])
async def publish_reel(job_id: str, req: ReelPublishRequest | None = None):
    reel: ReelMaker = _modules()["reel"]
    req = req or ReelPublishRequest()
    try:
        platforms = [Platform(p) for p in req.platforms] if req.platforms else None
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=f"Invalid platform: {exc}") from exc
    try:
        return await reel.publish(
            job_id,
            platforms=platforms,
...
@app.get("/api/v1/reel/jobs/{job_id}/video", tags=["REEL-MAKER"])
async def download_reel_video(job_id: str):
    reel: ReelMaker = _modules()["reel"]
    job = reel.get_job(job_id)
