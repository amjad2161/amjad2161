...
class ReelComposeRequest(BaseModel):
    topic: str = Field(..., min_length=1, max_length=500, description="Reel subject / niche")
    style: str = Field(
        "viral_hook",
        pattern="^(viral_hook|storytelling|tutorial|motivational|product|trend_remix|news_buzz)$",
    )
    platforms: list[str] = Field(
        default_factory=lambda: ["tiktok", "instagram"],
        description="Target platforms: tiktok, instagram, youtube, facebook",
    )
    duration_s: float | None = Field(None, ge=5.0, le=90.0)
    hook_type: str | None = Field(
        None,
        pattern="^(curiosity_gap|shock_stat|pov|before_after|listicle|controversy|question)$",
    )
